...
n = 2  # Maximum of 32 , only till 32, it is working for this topology
print 'NUMBER OF HOSTS - N =', n

of.OFP_SSL_PORT=6634
of.OFP_TCP_PORT=6634
    ...
    if event.connection.dpid == s1_dpid:
        ...
        if a:
            ...
            if a_protodst_str.startswith('10.0.0.'):
                ...
                if i % 2 == 1:
                    forward_ARP(event, (i + 1) / 2+offset)
                else:
                    forward_ARP(event, (n / 2) + 1+offset)

        for i in range(n + 1)[1:]:
            if i % 2 == 1:
                send_InstallFlow_msg_IPdst_match(event, 10, '10.0.0.{}'.format(i), (i + 1) / 2+offset)
            else:
                ...
                if i % 4 == 0:
                    ...
                    # SD flow
                    send_InstallFlow_msg_IPdst_match(event, 10, '10.0.0.{}'.format(i), (n / 2) + 1+offset, 1)
        ...
        if a:
            ...
            if a_protodst_str.startswith('10.0.0.'):
                ...
                if i % 2 == 1:
                    ...
                    forward_ARP(event, i / 2+offset)

        for i in range(n + 1)[1:]:
            if i % 2 == 1:
                send_InstallFlow_msg_IPdst_match(event, 10, '10.0.0.{}'.format(i), (n / 2) + 1+offset)
            else:
                send_InstallFlow_msg_IPdst_match(event, 10, '10.0.0.{}'.format(i), i / 2+offset)
